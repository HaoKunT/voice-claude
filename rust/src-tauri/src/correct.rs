...
use crate::config::{Config, CORRECT_MODE_CLOUD, CORRECT_MODE_OFF, CORRECT_MODE_OLLAMA, CORRECT_MODE_OPENROUTER};
use anyhow::{Context, Result};
use serde::{Deserialize, Serialize};
use std::time::Duration;

const CORRECTION_PROMPT: &str = "你是一个语音识别纠错助手。用户通过语音输入文字，可能有同音字错误、漏字、多字等问题。
...
async fn correct_openrouter(text: &str, cfg: &Config) -> Result<String> {
    ...
    let model = if cfg.correct_model.is_empty() { "qwen/qwen3-8b" } else { &cfg.correct_model };

    let body = OpenAIRequest {
        model,
        messages: vec![
            OpenAIMessage { role: "system", content: CORRECTION_PROMPT },
    ...
    Ok(parsed.choices.first().map(|c| c.message.content.trim().to_string()).unwrap_or_else(|| text.to_string()))
}
...
async fn correct_cloud(text: &str, cfg: &Config) -> Result<String> {
    let url = if cfg.correct_url.ends_with("/chat/completions") {
        ...
        format!("{}/v1/chat/completions", cfg.correct_url.trim_end_matches('/'))
    };
    let body = OpenAIRequest {
...
async fn correct_ollama(text: &str, cfg: &Config) -> Result<String> {
    ...
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(cfg.correct_timeout_secs()))
        .build()?;
    let resp = client.post(&cfg.correct_url).json(&req).send().await.context("call ollama")?;
    let status = resp.status();
    let text_body = truncate_body(resp.text().await.unwrap_or_default());
    if !status.is_success() {
...
pub async fn check_ollama(url: &str) -> Result<()> {
    let base = if url.is_empty() { "http://localhost:11434" } else { url };
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(2))
        .build()?;
    let resp = client.get(format!("{}/api/tags", base.trim_end_matches('/'))).send().await?;
